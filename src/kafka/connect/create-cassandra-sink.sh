...
curl -s \
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.faker.kafkapipeline.fakerdata.mapping": "name=value.name, address=value.address, year=value.year,country=value.country,company=value.company,job=value.job,prefix=value.prefix,color_name=value.color_name,ascii_email=value.ascii_email,phone_number=value.phone_number",
    "topic.faker.kafkapipeline.fakerdata.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Done."

echo "Starting Math Fact Sink"
curl -s \
     -X POST http://localhost:8083/connectors \
     -H "Content-Type: application/json" \
     -d '{
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.mathfact.kafkapipeline.mathfactdata.mapping": "number=value.number,text=value.text",
    "topic.mathfact.kafkapipeline.mathfactdata.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Done."